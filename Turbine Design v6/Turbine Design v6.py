...
def info_input():
    ...
    info_input_Gui_posx = (info_input_Gui.winfo_screenwidth() // 2) - (info_input_Gui_width // 2)
    info_input_Gui_posy = (info_input_Gui.winfo_screenheight() // 2) - (info_input_Gui_height // 2) - 50
    info_input_Gui.geometry('{}x{}+{}+{}'.format(info_input_Gui_width, info_input_Gui_height, info_input_Gui_posx, info_input_Gui_posy))
...
def info_hypth():
    ...
    info_hypth_Gui_posx = (info_hypth_Gui.winfo_screenwidth() // 2) - (info_hypth_Gui_width // 2)
    info_hypth_Gui_posy = (info_hypth_Gui.winfo_screenheight() // 2) - (info_hypth_Gui_height // 2) - 50
    info_hypth_Gui.geometry('{}x{}+{}+{}'.format(info_hypth_Gui_width, info_hypth_Gui_height, info_hypth_Gui_posx, info_hypth_Gui_posy))
...
def info_veloc():
    ...
    info_veloc_Gui_posx = (info_veloc_Gui.winfo_screenwidth() // 2) - (info_veloc_Gui_width // 2)
    info_veloc_Gui_posy = (info_veloc_Gui.winfo_screenheight() // 2) - (info_veloc_Gui_height // 2) - 50
    info_veloc_Gui.geometry('{}x{}+{}+{}'.format(info_veloc_Gui_width, info_veloc_Gui_height, info_veloc_Gui_posx, info_veloc_Gui_posy))
    ...
    axs.text(0.0, 0.0, r"$\psi \: = \: \dfrac{2c_p\:\Delta T_0}{U^2}$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    axs.text(0.5, 0.0, r"$\phi \: = \: \dfrac{V_a}{U}$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    axs.text(1.0, 0.0, r"$\Lambda \: = \: \dfrac{T_2 - T_3}{T_1 - T_3}$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    plt.ylim(-0.5,1.5)
    plt.axis('off')
        ...
        for j in range(nrows-1,i,-1):
            for jj in range(nrows-1,i+nrows-j,-1):
                if (data_smith[2*k+1][jj] > data_smith[2*k+1][jj-1]):
                    data_smith[2*k][jj], data_smith[2*k][jj-1] = data_smith[2*k][jj-1], data_smith[2*k][jj]
                    data_smith[2*k+1][jj], data_smith[2*k+1][jj-1] = data_smith[2*k+1][jj-1], data_smith[2*k+1][jj]
    
    fig, axs = plt.subplots(figsize = (6,4))
    for i in range(0,7):
        axs.plot(data_smith[2*i], data_smith[2*i+1], 'k-')
        axs.text(data_smith[2*i][index_inflection[i]], data_smith[2*i+1][index_inflection[i]]+0.05, "%d%%" % (94-i), color = 'black')
    axs.set(xlabel = r"$\phi$ = V$_a$ / U", ylabel = r"$\psi$ = 2c$_p$$\Delta$T$_0$ / U$^2$")
...
def info_blade():
    ...
    info_blade_Gui_posx = (info_blade_Gui.winfo_screenwidth() // 2) - (info_blade_Gui_width // 2)
    info_blade_Gui_posy = (info_blade_Gui.winfo_screenheight() // 2) - (info_blade_Gui_height // 2) - 50
    info_blade_Gui.geometry('{}x{}+{}+{}'.format(info_blade_Gui_width, info_blade_Gui_height, info_blade_Gui_posx, info_blade_Gui_posy))
    ...
    axs.text(0.0, 0.0, r"$\dfrac{dh_0}{dr} \: = \: V_a \: \dfrac{dV_a}{dr} \: + \: V_w \: \dfrac{dV_w}{dr} \: + \: \dfrac{V_w^2}{r}$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    plt.xlim(-1.0,1.0)
    plt.ylim(-0.5,1.5)
    plt.axis('off')
    ...
    axs.text(-0.5, 0.0, r"$V_w r \: = \: constant$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    axs.text(+0.5, 0.0, r"$V_a \: = \: constant$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    plt.xlim(-1.0,1.0)
    plt.axis('off')
    ...
    axs.text(-0.5, 0.0, r"$\alpha_2 \: = \: constant$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    axs.text(+0.5, 0.0, r"$V_{w_2}\:r^{\:sin^2\:\alpha_2} \: = \: constant$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    plt.xlim(-1.0,1.0)
    plt.axis('off')
...
def info_pcnbl():
    ...
    info_pcnbl_Gui_posx = (info_pcnbl_Gui.winfo_screenwidth() // 2) - (info_pcnbl_Gui_width // 2)
    info_pcnbl_Gui_posy = (info_pcnbl_Gui.winfo_screenheight() // 2) - (info_pcnbl_Gui_height // 2) - 50
    info_pcnbl_Gui.geometry('{}x{}+{}+{}'.format(info_pcnbl_Gui_width, info_pcnbl_Gui_height, info_pcnbl_Gui_posx, info_pcnbl_Gui_posy))
...
def info_blpar():
    ...
    info_blpar_Gui_posx = (info_blpar_Gui.winfo_screenwidth() // 2) - (info_blpar_Gui_width // 2)
    info_blpar_Gui_posy = (info_blpar_Gui.winfo_screenheight() // 2) - (info_blpar_Gui_height // 2) - 50
    info_blpar_Gui.geometry('{}x{}+{}+{}'.format(info_blpar_Gui_width, info_blpar_Gui_height, info_blpar_Gui_posx, info_blpar_Gui_posy))
    ...
    axs.text(0.0, 0.0, r"$\alpha_2 \: or \: \beta_3 \: \approxeq \: \cos^{-1}{(o/s)}$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    plt.xlim(-1.0,1.0)
    plt.axis('off')
...
def info_stress():
    ...
    info_stress_Gui_posx = (info_stress_Gui.winfo_screenwidth() // 2) - (info_stress_Gui_width // 2)
    info_stress_Gui_posy = (info_stress_Gui.winfo_screenheight() // 2) - (info_stress_Gui_height // 2) - 50
    info_stress_Gui.geometry('{}x{}+{}+{}'.format(info_stress_Gui_width, info_stress_Gui_height, info_stress_Gui_posx, info_stress_Gui_posy))
    ...
    axs.text(0.0, +0.4, r"$(\sigma_{ct})_{max} \: = \: \dfrac{2}{3} \: \pi \: N^2 \: \rho_b \: (A_2+A_3)$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    axs.text(0.0, -0.4, r"$(\sigma_{gb})_{max} \: = \: \dfrac{\dot m \: (V_{w_2}+V_{w_3}) \: h_R}{2 \: z \: n_R \: c_R^3}$", horizontalalignment = 'center', color = 'black', fontsize = 14)
    plt.ylim(-0.6,0.6)
    plt.xlim(-1.0,1.0)
    plt.axis('off')
...
def graph_zBn():
    ...
    axs1.text(80, 900, r"z = $\dfrac{I_{xx}}{y_{max}}$ = $\dfrac{1}{B}$ $\left(10\:\dfrac{t}{c}\right)^n$", fontsize = 14)
    fig.tight_layout()
    return fig
...
def initial_screen(): #cria janela principal
    ...
    tutorial_label.pack()
    start_button = tk.Button(initial_window, text = "START", command = design_screen, font = ("times new roman",14))
    start_button.place(x = myGui_width//2-50, y = myGui_height-75, width = 100, height = 50)
...
def input_variables(): #define parâmetros da subjanela de input
    ...
    input_frame = tk.LabelFrame(column1_frame, text = " INPUT VARIABLES ", font = framelab_font)
    ...
    labinp11 = tk.Label(input_frame, text = "Mass Flow Rate:", font = standard_font)
    ...
    text1 = tk.StringVar()
    entinp1 = tk.Entry(input_frame, textvariable = text1, width = 10, justify = 'center', font = standard_font)
    ...
    labinp21 = tk.Label(input_frame, text = "Specific Shaft Work:", font = standard_font)
    ...
    text2 = tk.StringVar()
    entinp2 = tk.Entry(input_frame, textvariable = text2, width = 10, justify = 'center', font = standard_font)
    ...
    labinp31 = tk.Label(input_frame, text = "Inlet Total Temperature:", font = standard_font)
    ...
    text3 = tk.StringVar()
    entinp3 = tk.Entry(input_frame, textvariable = text3, width = 10, justify = 'center', font = standard_font)
    entinp3.grid(row = 2, column = 3)
    text3.set("1100")
    labinp33 = tk.Label(input_frame, text = "K", font = standard_font)
    labinp33.grid(row = 2, column = 4, sticky = 'w')
    
    labinp41 = tk.Label(input_frame, text = "Inlet Total Pressure:", font = standard_font)
    ...
    text4 = tk.StringVar()
    entinp4 = tk.Entry(input_frame, textvariable = text4, width = 10, justify = 'center', font = standard_font)
    ...
    labinp51 = tk.Label(input_frame, text = "Pressure Ratio:", font = standard_font)
    ...
    text5 = tk.StringVar()
    entinp5 = tk.Entry(input_frame, textvariable = text5, width = 10, justify = 'center', font = standard_font)
    entinp5.grid(row = 4, column = 3)
    text5.set("1.873")
    
    nxtbtn1 = tk.Button(input_frame, text = "Next", command = read_input, font = standard_font, width = 10)
    nxtbtn1.grid(row = 6, column = 0, columnspan = 5, sticky = 'e', padx = 5, pady = 5)
    
    infobtn1 = tk.Button(input_frame, text = "Info", command = info_input, font = standard_font, width = 10)
    infobtn1.grid(row = 6, column = 0, columnspan = 5, sticky = 'w', padx = 5, pady = 5)
    
    global labinp61
    labinp61 = tk.Label(input_frame, text = "Enter all variables, please.", font = standard_font)
...
def hypotheses(): #define parâmetros da subjanela de hipóteses
    ...
    hypth_frame = tk.LabelFrame(column1_frame, text = " HYPOTHESES ", font = framelab_font)
    ...
    labhyp11 = tk.Label(hypth_frame, text = "Rotational Speed:", font = standard_font)
    ...
    text6 = tk.StringVar()
    enthyp1 = tk.Entry(hypth_frame, textvariable = text6, width = 10, justify = 'center', font = standard_font)
    ...
    labhyp21 = tk.Label(hypth_frame, text = "Mean Blade Speed:", font = standard_font)
    ...
    text7 = tk.StringVar()
    enthyp2 = tk.Entry(hypth_frame, textvariable = text7, width = 10, justify = 'center', font = standard_font)
    ...
    labhyp31 = tk.Label(hypth_frame, text = "Inlet Flow Angle:", font = standard_font)
    ...
    text8 = tk.StringVar()
    enthyp3 = tk.Entry(hypth_frame, textvariable = text8, width = 10, justify = 'center', font = standard_font)
    enthyp3.grid(row = 2, column = 3)
    text8.set("0")
    labhyp33 = tk.Label(hypth_frame, text = "°", font = standard_font)
    labhyp33.grid(row = 2, column = 4, sticky = 'w')
    
    labhyp41 = tk.Label(hypth_frame, text = "Outlet Flow Angle:", font = standard_font)
    ...
    text9 = tk.StringVar()
    enthyp4 = tk.Entry(hypth_frame, textvariable = text9, width = 10, justify = 'center', font = standard_font)
    enthyp4.grid(row = 3, column = 3)
    text9.set("10")
    labhyp43 = tk.Label(hypth_frame, text = "°", font = standard_font)
    labhyp43.grid(row = 3, column = 4, sticky = 'w')
    
    labhyp51 = tk.Label(hypth_frame, text = "Nozzle Loss Coefficient:", font = standard_font)
    ...
    text10 = tk.StringVar()
    enthyp5 = tk.Entry(hypth_frame, textvariable = text10, width = 10, justify = 'center', font = standard_font)
    enthyp5.grid(row = 4, column = 3)
    text10.set("0.05")
    
    labhyp71 = tk.Label(hypth_frame, text = "Flow Coefficient:", font = standard_font)
    ...
    enthyp72 = tk.Entry(hypth_frame, textvariable = text11, width = 10, justify = 'center', font = standard_font)
    enthyp72.grid(row = 8, column = 3)
    text11.set("0.8")
    text12 = tk.StringVar()
    text13 = tk.StringVar()
    labhyp73 = tk.Label(hypth_frame, text = "\u03D52 =", font = standard_font)
    enthyp73 = tk.Entry(hypth_frame, textvariable = text12, width = 10, justify = 'center', font = standard_font)
    text12.set("0.7")
    labhyp74 = tk.Label(hypth_frame, text = "\u03D53 =", font = standard_font)
    enthyp74 = tk.Entry(hypth_frame, textvariable = text13, width = 10, justify = 'center', font = standard_font)
    text13.set("1.0")
    ...
    rdbhyp1 = tk.Radiobutton(hypth_frame, text = "Constant through the rotor", command = axial_velocity, variable = axial_velocity_flag, value = 1)
    rdbhyp1.grid(row = 6, column = 0, sticky = 'w')
    rdbhyp2 = tk.Radiobutton(hypth_frame, text = "Variable through the rotor", command = axial_velocity, variable = axial_velocity_flag, value = 2)
    rdbhyp2.grid(row = 7, column = 0, sticky = 'w')
    
    nxtbtn2 = tk.Button(hypth_frame, text = "Next", command = read_hypth, font = standard_font, width = 10)
    nxtbtn2.grid(row = 11, column = 0, columnspan = 5, sticky = 'e', padx = 5, pady = 5)
    
    infobtn2 = tk.Button(hypth_frame, text = "Info", command = info_hypth, font = standard_font, width = 10)
    infobtn2.grid(row = 11, column = 0, columnspan = 5, sticky = 'w', padx = 5, pady = 5)
    
    global labhyp81
    labhyp81 = tk.Label(hypth_frame, text = "Enter all variables, please.", font = standard_font)
...
def velocity_diagrams_mean(): #define cálculos do triangulo de velocidades baseado no input de variação da vel axial
    ...
    visc_air_temp_curve = interp1d(data_visc_air_temp[0],data_visc_air_temp[1],kind='linear')
    ...
    if (mean_frame_flag == False):
        ...
        veloc_frame = tk.LabelFrame(column2_frame, text = " VELOCITY DIAGRAMS ", font = framelab_font)
        ...
        infobtn0 = tk.Button(veloc_frame, text = "Learn More", command = info_veloc, font = standard_font, width = 10)
...
def velocity_thermoprop_table(P1,P2,P3): #apresenta as informações termodinâmicas
    ...
    table_data = [["T$_0$", "K", "%.1f" % P1.T0, "%.1f" % P2.T0, "%.1f" % P3.T0],
                  ...
                  ["p$_0$", "bar", "%.3f" % P1.p0, "%.3f" % P2.p0, "%.3f" % P3.p0],
                  ["p", "bar", "%.3f" % P1.p, "%.3f" % P2.p, "%.3f" % P3.p],
                  [r"$\rho$", "kg/m$^3$", "%.3f" % P1.rho, "%.3f" % P2.rho, "%.3f" % P3.rho],
                  ["V", "m/s", "%.1f" % P1.V, "%.1f" % P2.V, "%.1f" % P3.V],
                  ["V$_a$", "m/s", "%.1f" % P1.Va, "%.1f" % P2.Va, "%.1f" % P3.Va],
                  ["V$_w$", "m/s", "%.1f" % P1.Vu, "%.1f" % P2.Vu, "%.1f" % P3.Vu],
                  ["W", "m/s", "-", "%.1f" % P2.W, "%.1f" % P3.W],
                  ["W$_a$", "m/s", "-", "%.1f" % P2.Wa, "%.1f" % P3.Wa],
                  ["W$_w$", "m/s", "-", "%.1f" % P2.Wu, "%.1f" % P3.Wu],
                  ["U", "m/s", "-", "%.1f" % P2.U, "%.1f" % P3.U],
                  [r"$\alpha$", "°", "%.2f" % P1.alpha, "%.2f" % P2.alpha, "%.2f" % P3.alpha],
                  [r"$\beta$", "°", "-", "%.2f" % P2.beta, "%.2f" % P3.beta],
                  ["Ma", "-", "%.3f" % P1.Mach, "%.3f" % P2.Mach, "%.3f" % P3.Mach],
                  ["Ma$_r$$_e$$_l$", "-", "-", "%.3f" % P2.Machrel, "%.3f" % P3.Machrel]]
    table1 = table_mean.table(cellText = table_data, colLabels = label_cols, edges = 'horizontal', cellLoc = 'center', rowLoc = 'center', colLoc = 'center', loc = 'center')
...
def velocity_diagrams_mean_plot(P1,P2,P3): #figura triangulos de vel
    ...
    velocity_diagrams.quiver(origin_x, origin_y, direction_x, direction_y, color = ['black','red','blue','red','blue'], angles = 'xy', scale_units = 'xy', scale = 1)
    ...
    velocity_diagrams.text(P3.U-2*P3.Wu/3+10, -2*P3.Wa/3-10, '$W_3$', color = 'red')
    velocity_diagrams.text(-P3.Vu/3-30, -P3.Va/3-10, '$V_3$', color = 'blue')
    
    if (P2.Vu > P2.U):
        horizontal_line1 = lines.Line2D([P2.U,P2.Vu], [0,0], color = 'gray', linestyle = '--')
        velocity_diagrams.add_line(horizontal_line1)
    vertical_line1 = lines.Line2D([P2.Vu,P2.Vu], [-P2.Va,0], color = 'gray', linestyle = '--')
    velocity_diagrams.add_line(vertical_line1)
    if (P3.Vu > 0.0):
        horizontal_line2 = lines.Line2D([-P3.Vu,0], [0,0], color = 'gray', linestyle = '--')
        velocity_diagrams.add_line(horizontal_line2)
    vertical_line2 = lines.Line2D([-P3.Vu,-P3.Vu], [-P3.Va,0], color = 'gray', linestyle = '--')
    ...
    if (P2.beta > 0.0):
        ...
        tt1, tt2 = P2.beta, 0.0
    beta2_arc = ptc.Arc((P2.Vu,-P2.Va), 3*P2.Va/5, 3*P2.Va/5, angle = 90, theta1 = tt1, theta2 = tt2, color = 'red')
    velocity_diagrams.add_patch(beta2_arc)
    if (P2.alpha > 0.0):
        tt1, tt2 = 0.0, P2.alpha
    else:
        tt1, tt2 = P2.alpha, 0.0
    alpha2_arc = ptc.Arc((P2.Vu,-P2.Va), 2*P2.Va/5, 2*P2.Va/5, angle = 90, theta1 = tt1, theta2 = tt2, color = 'blue')
    velocity_diagrams.add_patch(alpha2_arc)
    if (P3.beta > 0.0):
        tt1, tt2 = -P3.beta, 0.0
    else:
        tt1, tt2 = 0.0, -P3.beta
    beta3_arc = ptc.Arc((-P3.Vu,-P3.Va), 2*P3.Va/5, 2*P3.Va/5, angle = 90, theta1 = tt1, theta2 = tt2, color = 'red')
    velocity_diagrams.add_patch(beta3_arc)
    if (P3.alpha > 0.0):
        tt1, tt2 = -P3.alpha, 0.0
    else:
        tt1, tt2 = 0.0, -P3.alpha
    alpha3_arc = ptc.Arc((-P3.Vu,-P3.Va), 3*P3.Va/5, 3*P3.Va/5, angle = 90, theta1 = tt1, theta2 = tt2, color = 'blue')
    velocity_diagrams.add_patch(alpha3_arc)
    
    velocity_diagrams.text(P2.Vu-(7*P2.Va/20)*np.sin(np.radians(P2.beta/2+6)), -P2.Va+(7*P2.Va/20)*np.cos(np.radians(P2.beta/2+6)), r'$\beta_2$', color = 'red')
    velocity_diagrams.text(P2.Vu-(3*P2.Va/10)*np.sin(np.radians((P2.alpha+P2.beta)/2+6)), -P2.Va+(3*P2.Va/10)*np.cos(np.radians((P2.alpha+P2.beta)/2+6)), r'$\alpha_2$', color = 'blue')
    velocity_diagrams.text(-P3.Vu+(5*P3.Va/20)*np.sin(np.radians((P3.alpha+P3.beta)/2-6)), -P3.Va+(5*P3.Va/20)*np.cos(np.radians((P3.alpha+P3.beta)/2-6)), r'$\beta_3$', color = 'red')
    velocity_diagrams.text(-P3.Vu+(7*P3.Va/20)*np.sin(np.radians(P3.alpha/2-6)), -P3.Va+(7*P3.Va/20)*np.cos(np.radians(P3.alpha/2-6)), r'$\alpha_3$', color = 'blue')
    
    if (axial_velocity_flag.get() == 1):
        plt.title(label = r"$\phi$ = %.3f ; $\psi$ = %.3f ; $\Lambda$ = %.3f" % (phi,psi,dor), loc = 'center', fontsize = 12)
    else:
        plt.title(label = r"$\phi_2$ = %.3f ; $\phi_3$ = %.3f ; $\psi$ = %.3f ; $\Lambda$ = %.3f" % (phi2,phi3,psi,dor), loc = 'center', fontsize = 12)
    plt.axis('equal')
...
def blade_design(): #define parâmetros da subjanela de blade design
    global blade_frame
    blade_frame = tk.LabelFrame(column1_frame, text = " BLADE DESIGN METHOD ", font = framelab_font)
    ...
    rdbpar1 = tk.Radiobutton(blade_frame, text = "Free Vortex Design", variable = blade_design_flag, value = 1)
    rdbpar1.grid(row = 0, column = 0, columnspan = 5, sticky = 'w')
    rdbpar2 = tk.Radiobutton(blade_frame, text = "Constant Nozzle Angle Design", variable = blade_design_flag, value = 2)
    rdbpar2.grid(row = 1, column = 0, columnspan = 5, sticky = 'w')
    rdbpar3 = tk.Radiobutton(blade_frame, text = "First Power Design", variable = blade_design_flag, value = 3)
    rdbpar3.grid(row = 2, column = 0, columnspan = 5, sticky = 'w')
    
    nxtbtn3 = tk.Button(blade_frame, text = "Next", command = velocity_diagrams_root_tip, font = standard_font, width = 10)
    nxtbtn3.grid(row = 3, column = 0, columnspan = 5, sticky = 'e', padx = 5, pady = 5)

    infobtn3 = tk.Button(blade_frame, text = "Info", command = info_blade, font = standard_font, width = 10)
...
def velocity_diagrams_root_tip():
    ...
    P1h, P2h, P3h = calculate_tip_hub(G2.rh,G3.rh,G2.rm,G3.rm,P1m,P2m,P3m)
    P1t, P2t, P3t = calculate_tip_hub(G2.rt,G3.rt,G2.rm,G3.rm,P1m,P2m,P3m)
    
    global column2_frame, choke_frame, root_frame, tip_frame, root_tip_frame_flag
    if (root_tip_frame_flag == False):
        choke_frame = tk.LabelFrame(column2_frame, text = " CHOKE CONDITION ", font = framelab_font)
...
def graph_Mach(Mach_numbers):
    ...
    colors = ['xkcd:green','xkcd:red','xkcd:blue',
              ...
              'xkcd:green','xkcd:red','xkcd:blue']
    labels = ["","Station 1\n(Absolute)","","","Station 1\n(Absolute)","","","Station 2\n(Relative)","","","Station 3\n(Relative)","",]
    axs.bar(locations, Mach_numbers, color = colors, width = 0.2, tick_label = labels)
    ...
    axs.legend((root_leg,mean_leg,tip_leg), ("Root","Mean","Tip"), loc = 'upper left')
    axs.spines['top'].set_visible(False)
    axs.spines['right'].set_visible(False)
    axs.set_ylim(0,1.2)
    axs.set_xlim(0,2.9)
...
def calculate_tip_hub(r2,r3,r2m,r3m,P1m_,P2m_,P3m_):
    ...
    if (blade_design_flag.get() == 1):
        P2.alpha = np.degrees(np.arctan((r2m/r2)*np.tan(np.radians(P2m_.alpha))))
        P2.beta = np.degrees(np.arctan((r2m/r2)*np.tan(np.radians(P2m_.alpha))-(r2/r2m)*(P2m_.U/P2m_.Va)))
        P3.alpha = np.degrees(np.arctan((r3m/r3)*np.tan(np.radians(P3m_.alpha))))
        P3.beta = np.degrees(np.arctan((r3m/r3)*np.tan(np.radians(P3m_.alpha))+(r3/r3m)*(P3m_.U/P3m_.Va)))
...
def velocity_diagrams_root_tip_plot(P1,P2,P3):
    ...
    velocity_diagrams.quiver(origin_x, origin_y, direction_x, direction_y, color = ['black','red','blue', 'black', 'red','blue'], angles = 'xy', scale_units = 'xy', scale = 1)
    
    velocity_diagrams.text(offset_x + P2.U/2-20, 10, '$U_2$', color = 'black')
    velocity_diagrams.text(offset_x + P2.U+P2.Wu/3+20, -P2.Wa/3-20, '$W_2$', color = 'red')
    velocity_diagrams.text(offset_x + 2*P2.Vu/3-40, -2*P2.Va/3-20, '$V_2$', color = 'blue')
    velocity_diagrams.text(P3.U/2-20, 10, '$U_3$', color = 'black')
    velocity_diagrams.text(P3.U-2*P3.Wu/3+20, -2*P3.Wa/3-20, '$W_3$', color = 'red')
    velocity_diagrams.text(-P3.Vu/3-40, -P3.Va/3-20, '$V_3$', color = 'blue')
    
    if (P2.beta > 0.0):
        horizontal_line1 = lines.Line2D([offset_x + P2.U,offset_x + P2.Vu], [0,0], color = 'gray', linestyle = '--')
        velocity_diagrams.add_line(horizontal_line1)
    vertical_line1 = lines.Line2D([offset_x + P2.Vu,offset_x + P2.Vu], [-P2.Va,0], color = 'gray', linestyle = '--')
    velocity_diagrams.add_line(vertical_line1)
    if (P3.alpha > 0.0):
        horizontal_line2 = lines.Line2D([-P3.Vu,0], [0,0], color = 'gray', linestyle = '--')
        velocity_diagrams.add_line(horizontal_line2)
    ...
    if (P2.beta > 0.0):
        ...
        tt1, tt2 = P2.beta, 0.0
    beta2_arc = ptc.Arc((offset_x + P2.Vu,-P2.Va), 3*P2.Va/5, 3*P2.Va/5, angle = 90, theta1 = tt1, theta2 = tt2, color = 'red')
    velocity_diagrams.add_patch(beta2_arc)
    if (P2.alpha > 0.0):
        tt1, tt2 = 0.0, P2.alpha
    else:
        tt1, tt2 = P2.alpha, 0.0
    alpha2_arc = ptc.Arc((offset_x + P2.Vu,-P2.Va), 2*P2.Va/5, 2*P2.Va/5, angle = 90, theta1 = tt1, theta2 = tt2, color = 'blue')
    velocity_diagrams.add_patch(alpha2_arc)
    if (P3.beta > 0.0):
        tt1, tt2 = -P3.beta, 0.0
    else:
    ...
    velocity_diagrams.text(offset_x + P2.Vu-(7*P2.Va/20)*np.sin(np.radians(P2.beta/2+9)), -P2.Va+(7*P2.Va/20)*np.cos(np.radians(P2.beta/2+9)), r'$\beta_2$', color = 'red')
    velocity_diagrams.text(offset_x + P2.Vu-(7*P2.Va/20)*np.sin(np.radians((P2.alpha+P2.beta)/2+9)), -P2.Va+(3*P2.Va/10)*np.cos(np.radians((P2.alpha+P2.beta)/2+9)), r'$\alpha_2$', color = 'blue')
    velocity_diagrams.text(-P3.Vu+(5*P3.Va/20)*np.sin(np.radians((P3.alpha+P3.beta)/2-9)), -P3.Va+(5*P3.Va/20)*np.cos(np.radians((P3.alpha+P3.beta)/2-9)), r'$\beta_3$', color = 'red')
    velocity_diagrams.text(-P3.Vu+(7*P3.Va/20)*np.sin(np.radians(P3.alpha/2-9)), -P3.Va+(7*P3.Va/20)*np.cos(np.radians(P3.alpha/2-9)), r'$\alpha_3$', color = 'blue')
...
def pitch_chord_nblades_p1(): #calculos pitch/chord height/chord
    ...
    if (pcnbl_frame_flag == False):
        ...
        pcnbl_frame = tk.LabelFrame(column3_frame, text = " PITCH, CHORD AND NUMBER OF BLADES ", font = framelab_font)
    ...
    nozzle.h, nozzle.rm, nozzle.rt, nozzle.rh, nozzle.rr = calculate_blades(nozzle,G1,G2)
    rotor.h, rotor.rm, rotor.rt, rotor.rh, rotor.rr = calculate_blades(rotor,G2,G3)
    ...
    if (pcnbl_frame_flag == False):
        ...
        labpcn21 = tk.Label(pcnbl_frame, text = "Nozzle blade aspect ratio:", font = standard_font)
        ...
        text15 = tk.StringVar()
        entinp2 = tk.Entry(pcnbl_frame, textvariable = text15, width = 10, justify = 'center', font = standard_font)
        entinp2.grid(row = 1, column = 3)
        text15.set("3.0")
        labpcn31 = tk.Label(pcnbl_frame, text = "Rotor blade aspect ratio:", font = standard_font)
        ...
        text16 = tk.StringVar()
        entinp3 = tk.Entry(pcnbl_frame, textvariable = text16, width = 10, justify = 'center', font = standard_font)
        entinp3.grid(row = 2, column = 3)
        text16.set("3.0")
        labpcn41 = tk.Label(pcnbl_frame, text = "Rotor blade height-width ratio:", font = standard_font)
        ...
        text17 = tk.StringVar()
        entpcn4 = tk.Entry(pcnbl_frame, textvariable = text17, width = 10, justify = 'center', font = standard_font)
        entpcn4.grid(row = 3, column = 3)
        text17.set("3.0")
        labpcn51 = tk.Label(pcnbl_frame, text = "Space between nozzle and rotor blades:", font = standard_font)
        ...
        text18 = tk.StringVar()
        entpcn5 = tk.Entry(pcnbl_frame, textvariable = text18, width = 10, justify = 'center', font = standard_font)
        entpcn5.grid(row = 4, column = 3)
        text18.set("0.25")
        
        nxtbtn4 = tk.Button(pcnbl_frame, text = "Next", command = pitch_chord_nblades_p2, font = standard_font, width = 10)
        nxtbtn4.grid(row = 6, column = 0, columnspan = 4, sticky = 'e', padx = 5, pady = 5)
        
        infobtn4 = tk.Button(pcnbl_frame, text = "Info", command = info_pcnbl, font = standard_font, width = 10)
        infobtn4.grid(row = 6, column = 0, columnspan = 4, sticky = 'w', padx = 5, pady = 5)
        
        global labpcn61
        labpcn61 = tk.Label(pcnbl_frame, text = "Enter all variables, please.", font = standard_font)
...
def pitch_chord_nblades_p2():
    ...
    if ((not text15.get()) | (not text16.get()) | (not text17.get()) | (not text18.get())):
        ...
        flare_angle = 2.0*np.degrees(np.arctan(((G3.h - G2.h)/2.0)/(sbnr + rotor.w)))
        nozzle.w = ((G2.h - G1.h)/2.0)/np.tan(np.radians(flare_angle/2.0)) - sbnr
        ...
        if (sktch_frame_flag == False):
            ...
            sktch_frame = tk.LabelFrame(column4_frame, text = " STAGE SKETCH ", font = framelab_font)
            sktch_frame.grid(row = 0, column = 0, padx = 10, pady = 10)
        
        global iteration_flag
        if (iteration_flag == 0):
            figure8 = sketch_stage(nozzle.h,rotor.h,nozzle.w,rotor.w,sbnr,flare_angle)
            ...
            table_data = [["r$_m$", "m", "%.4f" % nozzle.rm, "%.4f" % rotor.rm],
                          ...
                          [r"$\gamma$", "°", "%.2f" % flare_angle, "%.2f" % flare_angle],
                          ["c", "m", "%.4f" % nozzle.c, "%.4f" % rotor.c],
                          ["s", "m", "%.4f" % nozzle.s, "%.4f" % rotor.s],
                          ["(h/c)", "-", "%.3f" % nozzle.hc, "%.3f" % rotor.hc],
                          ["(s/c)", "-", "%.3f" % nozzle.sc, "%.3f" % rotor.sc],
                          ["n", "-", "%d" % nozzle.n, "%d" % rotor.n]]
            table1 = blade_parameters.table(cellText = table_data, colLabels = label_cols, edges = 'horizontal', cellLoc = 'center', rowLoc = 'center', colLoc = 'center', loc = 'center')
            table1.auto_set_font_size(False)
            table1.set_fontsize(10)
...
def calculate_optsc(inlet,outlet):
    ...
    interp1 = np.poly1d(np.polyfit(data_sc[2*in_lower],data_sc[2*in_lower+1],3))
    interp2 = np.poly1d(np.polyfit(data_sc[2*in_upper],data_sc[2*in_upper+1],3))
    optimum_sc = interp1(outlet) + (interp2(outlet)-interp1(outlet))*(inlet-in_lower*10)/(in_upper*10-in_lower*10)
    return optimum_sc
...
def graph_optsc():
    ...
    for i in range(0,7):
        ...
        axs.text(data_sc[2*i][0]-2.5, data_sc[2*i+1][0]-0.01, "%d°" % (i*10), color = 'black')
    axs.set(xlabel = r"Relative efflux gas angle ($\alpha_2$ or $\beta_3$)", ylabel = "'Optimum' s/c")
    axs.spines['top'].set_visible(False)
    axs.spines['right'].set_visible(False)
    plt.xlim(35,75)
    plt.ylim(0.5,1.1)
    ...
    nozzle_v = lines.Line2D([P2m.alpha,P2m.alpha], [0.5,nozzle.sc], color = 'blue', linestyle = '--', alpha = 0.5)
    axs.add_line(nozzle_v)
    nozzle_h = lines.Line2D([35,P2m.alpha], [nozzle.sc,nozzle.sc], color = 'blue', linestyle = '--', alpha = 0.5)
    axs.add_line(nozzle_h)
    axs.scatter(P3m.beta, rotor.sc, c = 'g', marker = 'o', alpha = 0.5)
    rotor_v = lines.Line2D([P3m.beta,P3m.beta], [0.5,rotor.sc], color = 'green', linestyle = '--', alpha = 0.5)
    axs.add_line(rotor_v)
    rotor_h = lines.Line2D([35,P3m.beta], [rotor.sc,rotor.sc], color = 'green', linestyle = '--', alpha = 0.5)
    axs.add_line(rotor_h)
    
    axs.set_xticklabels('{:.0f}°'.format(i) for i in axs.get_xticks())
    nozzle_leg = lines.Line2D([], [], color = 'blue', linestyle = '--', alpha = 0.5, marker = 'o')
    rotor_leg = lines.Line2D([], [], color = 'green', linestyle = '--', alpha = 0.5, marker = 'o')
    axs.legend((nozzle_leg,rotor_leg), ("(s/c)$_N$ = %.3f" % nozzle.sc,"(s/c)$_R$ = %.3f" % rotor.sc), loc = 'upper right')
    return fig
...
def sketch_stage(hN,hR,wN,wR,sNR,flr):
    ...
    ypos2 = 0.0
    auxiliary_line1 = lines.Line2D([xpos1,xpos2], [ypos1,ypos2], color = 'gray', linestyle = '--')
    sketch.add_line(auxiliary_line1)
    auxiliary_line2 = lines.Line2D([xpos1,xpos2], [-ypos1,-ypos2], color = 'gray', linestyle = '--')
    sketch.add_line(auxiliary_line2)
    xpos3 = 3.0*sNR + wN + wR
    ypos3 = 0.0
    auxiliary_line3 = lines.Line2D([xpos2,xpos3], [ypos2,ypos3], color = 'gray', linestyle = '-.')
    sketch.add_line(auxiliary_line3)
    flare_arc = ptc.Arc((xpos2,ypos2), np.sqrt(xpos2**2.0 + ypos1**2.0)/2.0, np.sqrt(xpos2**2.0 + ypos1**2.0)/2.0, angle = 0, theta1 = -flr/2.0, theta2 = flr/2.0, color = 'black')
    sketch.add_patch(flare_arc)
    sketch.text(-5.0*np.sqrt(xpos2**2.0 + ypos1**2.0)/8.0, 0.0, r"$\gamma$", color = 'black')
    
    xpos1 = 0.0
    ypos1 = hN/2.0 - (sNR + wN/2.0)*np.tan(np.radians(flr/2.0))
    xpos2 = 3.0*sNR + wN + wR
    ypos2 = hR/2.0 + (sNR + wR/2.0)*np.tan(np.radians(flr/2.0))
    wall_line1 = lines.Line2D([xpos1,xpos2], [ypos1,ypos2], color = 'black', linestyle = '-')
    sketch.add_line(wall_line1)
    wall_line2 = lines.Line2D([xpos1,xpos2], [-ypos1,-ypos2], color = 'black', linestyle = '-')
    sketch.add_line(wall_line2)
    
    xpos1 = sNR
    ypos1 = hN/2.0 - (wN/2.0)*np.tan(np.radians(flr/2.0))
    nozzle_line1 = lines.Line2D([xpos1,xpos1], [ypos1,-ypos1], color = 'black', linestyle = '-')
    sketch.add_line(nozzle_line1)
    xpos2 = sNR + wN
    ypos2 = hN/2.0 + (wN/2.0)*np.tan(np.radians(flr/2.0))
    nozzle_line2 = lines.Line2D([xpos2,xpos2], [ypos2,-ypos2], color = 'black', linestyle = '-')
    ...
    rotor_line1 = lines.Line2D([xpos1,xpos1], [ypos1-0.02*hR,-ypos1], color = 'black', linestyle = '-')
    sketch.add_line(rotor_line1)
    xpos2 = 2.0*sNR + wN + wR
    ypos2 = hR/2.0 + (wR/2.0)*np.tan(np.radians(flr/2.0))
    rotor_line2 = lines.Line2D([xpos2,xpos2], [ypos2-0.02*hR,-ypos2], color = 'black', linestyle = '-')
    sketch.add_line(rotor_line2)
    rotor_line3 = lines.Line2D([xpos1,xpos2], [ypos1-0.02*hR,ypos2-0.02*hR], color = 'black', linestyle = '-')
    ...
    xpos1 = sNR/2.0
    ypos1 = G1.h/2.0
    stations_line1 = lines.Line2D([xpos1,xpos1], [ypos1+0.2*G1.h,-(ypos1+ + 0.2*G1.h)], color = 'blue', linestyle = '--')
    sketch.add_line(stations_line1)
    sketch.text(xpos1-0.002, ypos1+0.3*G1.h, "1", color = 'blue')
    xpos1 = 3.0*sNR/2.0 + wN
    ypos1 = G2.h/2.0
    stations_line2 = lines.Line2D([xpos1,xpos1], [ypos1+0.2*G1.h,-(ypos1+0.2*G1.h)], color = 'blue', linestyle = '--')
    sketch.add_line(stations_line2)
    sketch.text(xpos1-0.002, ypos1+0.3*G1.h, "2", color = 'blue')
    xpos1 = 5.0*sNR/2.0 + wN + wR
    ypos1 = G3.h/2.0
    stations_line3 = lines.Line2D([xpos1,xpos1], [ypos1+0.2*G1.h,-(ypos1+0.2*G1.h)], color = 'blue', linestyle = '--')
    sketch.add_line(stations_line3)
    sketch.text(xpos1-0.002, ypos1+0.3*G1.h, "3", color = 'blue')
    
    cline1 = lines.Line2D([1.5*sNR + wN/2.0, 6.0*sNR + wN + wR], [hN/2.0, hN/2.0], color = 'gray', linestyle = '-')
    sketch.add_line(cline1)
    cline2 = lines.Line2D([1.5*sNR + wN/2.0, 6.0*sNR + wN + wR], [-hN/2.0, -hN/2.0], color = 'gray', linestyle = '-')
    sketch.add_line(cline2)
    cline3 = lines.Line2D([2.5*sNR + wN + wR/2.0, 9.0*sNR + wN + wR], [hR/2.0-0.02*hR, hR/2.0-0.02*hR], color = 'gray', linestyle = '-')
    sketch.add_line(cline3)
    cline4 = lines.Line2D([2.5*sNR + wN + wR/2.0, 9.0*sNR + wN + wR], [-hR/2.0, -hR/2.0], color = 'gray', linestyle = '-')
    sketch.add_line(cline4)
    origin_x = [5.0*sNR + wN + wR, 5.0*sNR + wN + wR, 8.0*sNR + wN + wR, 8.0*sNR + wN + wR]
    origin_y = [0.0, 0.0, 0.0, 0.0]
    direction_x = [0.0, 0.0, 0.0, 0.0]
    direction_y = [hN/2.0, -hN/2.0, hR/2.0-0.02*hR, -hR/2.0]
    sketch.quiver(origin_x, origin_y, direction_x, direction_y, color = ['gray','gray','gray','gray'], angles = 'xy', scale_units = 'xy', scale = 1)
    ...
    xpos1 = sNR
    ypos1 = hN/2.0 - (wN/2.0)*np.tan(np.radians(flr/2.0))
    cline5 = lines.Line2D([xpos1,xpos1], [-ypos1-0.5*sNR,-ypos2-4.0*sNR], color = 'gray', linestyle = '-')
    sketch.add_line(cline5)
    xpos1 = sNR + wN
    ypos1 = hN/2.0 + (wN/2.0)*np.tan(np.radians(flr/2.0))
    cline6 = lines.Line2D([xpos1,xpos1], [-ypos1-0.5*sNR,-ypos2-4.0*sNR], color = 'gray', linestyle = '-')
    sketch.add_line(cline6)
    xpos1 = 2.0*sNR + wN
    ypos1 = hR/2.0 - (wR/2.0)*np.tan(np.radians(flr/2.0))
    cline7 = lines.Line2D([xpos1,xpos1], [-ypos1-0.5*sNR,-ypos2-4.0*sNR], color = 'gray', linestyle = '-')
    sketch.add_line(cline7)
    xpos1 = 2.0*sNR + wN + wR
    ypos1 = hR/2.0 + (wR/2.0)*np.tan(np.radians(flr/2.0))
    cline8 = lines.Line2D([xpos1,xpos1], [-ypos1-0.5*sNR,-ypos2-4.0*sNR], color = 'gray', linestyle = '-')
    sketch.add_line(cline8)
    cline9 = lines.Line2D([sNR + wN,2.0*sNR + wN], [-ypos1-3.5*sNR,-ypos2-3.5*sNR], color = 'gray', linestyle = '-')
    ...
    sketch.text(1.5*sNR + wN - 0.005, -ypos2-5.0*sNR, "s$_N$$_R$", color = 'gray')
    sketch.text(2.0*sNR + wN + wR/2.0 - 0.005, -ypos2-5.0*sNR, "w$_R$", color = 'gray')
...
def other_blade_parameters():
    global blpar_frame
    blpar_frame = tk.LabelFrame(column4_frame, text = " OTHER BLADE PARAMETERS ", font = framelab_font)
    ...
    labbpr11 = tk.Label(blpar_frame, text = "Maximum blade thickness to chord ratio:", font = standard_font)
    ...
    text19 = tk.StringVar()
    entbpr1 = tk.Entry(blpar_frame, textvariable = text19, width = 10, justify = 'center', font = standard_font)
    ...
    text20 = tk.StringVar()
    entbpr2 = tk.Entry(blpar_frame, textvariable = text20, width = 10, justify = 'center', font = standard_font)
    entbpr2.grid(row = 2, column = 3)
    text20.set("0.20")
    
    shroud_flag = tk.IntVar(value = 1)
    labbprrb = tk.Label(blpar_frame, text = "Type of Rotor Tip:", font = standard_font)
    labbprrb.grid(row = 3, column = 0, sticky = 'w')
    rdbpar1 = tk.Radiobutton(blpar_frame, text = "Unshrouded", variable = shroud_flag, value = 1)
    rdbpar1.grid(row = 4, column = 0, columnspan = 5, sticky = 'w')
    rdbpar2 = tk.Radiobutton(blpar_frame, text = "Shrouded", variable = shroud_flag, value = 2)
    rdbpar2.grid(row = 5, column = 0, columnspan = 5, sticky = 'w')
    
    labbpr41 = tk.Label(blpar_frame, text = "Tip clearance to height ratio:", font = standard_font)
    ...
    text21 = tk.StringVar()
    entbpr3 = tk.Entry(blpar_frame, textvariable = text21, width = 10, justify = 'center', font = standard_font)
    entbpr3.grid(row = 7, column = 3)
    text21.set("0.02")
    
    labbpr61 = tk.Label(blpar_frame, text = "Trailing edge thickness to pitch ratio:", font = standard_font)
    ...
    text22 = tk.StringVar()
    entbpr4 = tk.Entry(blpar_frame, textvariable = text22, width = 10, justify = 'center', font = standard_font)
    ...
    text23 = tk.StringVar()
    entbpr5 = tk.Entry(blpar_frame, textvariable = text23, width = 10, justify = 'center', font = standard_font)
    entbpr5.grid(row = 10, column = 3)
    text23.set("0.02")
    
    labbpr91 = tk.Label(blpar_frame, text = "Trailing edge thickness to opening ratio:", font = standard_font)
    ...
    text24 = tk.StringVar()
    entbpr6 = tk.Entry(blpar_frame, textvariable = text24, width = 10, justify = 'center', font = standard_font)
    ...
    text25 = tk.StringVar()
    entbpr7 = tk.Entry(blpar_frame, textvariable = text25, width = 10, justify = 'center', font = standard_font)
    entbpr7.grid(row = 13, column = 3)
    text25.set("0.04")
    
    nxtbtn5 = tk.Button(blpar_frame, text = "Next", command = stress_analysis, font = standard_font, width = 10)
    nxtbtn5.grid(row = 15, column = 0, columnspan = 4, sticky = 'e', padx = 5, pady = 5)
    
    infobtn5 = tk.Button(blpar_frame, text = "Info", command = info_blpar, font = standard_font, width = 10)
    infobtn5.grid(row = 15, column = 0, columnspan = 4, sticky = 'w', padx = 5, pady = 5)
    
    global labbpr121
    labbpr121 = tk.Label(blpar_frame, text = "Enter all variables, please.", font = standard_font)
...
def stress_analysis():
    ...
    if ((not text19.get()) | (not text20.get()) | (not text21.get()) | (not text22.get()) | (not text23.get()) | (not text24.get()) | (not text25.get())):
        ...
        if (stress_frame_flag == False):
            stress_frame = tk.LabelFrame(column3_frame, text = " STRESS ANALYSIS ", font = framelab_font)
        ...
        n = n_curve(camber)
        z = (10.0*rotor.tc)**n/B
        sigma_gb = (m*(P2m.Vu + P3m.Vu)*rotor.h/(2.0*rotor.n*z*rotor.c**3.0))/10**6.0
        ...
        if (stress_frame_flag == False):
            nxtbtn6 = tk.Button(stress_frame, text = "Next", command = loss_models_screen, font = standard_font, width = 10)
            nxtbtn6.grid(row = 1, column = 0, columnspan = 4, sticky = 'e', padx = 5, pady = 5)
            
            infobtn6 = tk.Button(stress_frame, text = "Info", command = info_stress, font = standard_font, width = 10)
            infobtn6.grid(row = 1, column = 0, columnspan = 4, sticky = 'w', padx = 5, pady = 5)
...
def graph_stress():
    ...
    axs.text(np.amax(data_stress[0])-30, np.amin(data_stress[1])+4, "1100 K", rotation = -27, color = 'black')
    axs.text(np.amax(data_stress[2])-30, np.amin(data_stress[3])+5, "1200 K", rotation = -35, color = 'black')
    axs.text(np.amax(data_stress[4])-20, np.amin(data_stress[5])+6, "1300 K", rotation = -55, color = 'black')
    axs.set(xlabel = r"Permissible $\sigma$$_c$$_t$ [MN/m$^2$]", ylabel = r"Permissible $\sigma$$_g$$_b$ [MN/m$^2$]")
    ...
    vertical_line = lines.Line2D([sigma_ct,sigma_ct], [0,sigma_gb], color = 'red', linestyle = '--', alpha = 0.5)
    axs.add_line(vertical_line)
    horizontal_line = lines.Line2D([0,sigma_ct], [sigma_gb,sigma_gb], color = 'red', linestyle = '--', alpha = 0.5)
...
def loss_models_screen():
    ...
    if (losses_frame_flag == False):
        ...
        myGui2_posy = (myGui2.winfo_screenheight() // 2) - (myGui2_height // 2) - 50
        myGui2.geometry('{}x{}+{}+{}'.format(myGui2_width, myGui2_height, myGui2_posx, myGui2_posy))
...
def design_point_losses():
    ...
    design_point_AM, design_point_DC, design_point_KO = loss_models(P1m,P2m,P3m,P1h.Mach,P2h.Machrel)
    
    global dloss_frame, noteb_dloss, losses_frame_flag
    global AM_frame, DC_frame, KO_frame
    if (losses_frame_flag == False):
        dloss_frame = tk.LabelFrame(losses_window, text = " DESIGN POINT LOSSES ", font = framelab_font)
        ...
        AM_btn = tk.Button(AM_frame, text = "Choose Model to Iterate", command = AM_iteration, font = ("times new roman",12))
        ...
        DC_btn = tk.Button(DC_frame, text = "Choose Model to Iterate", command = DC_iteration, font = ("times new roman",12))
        ...
        KO_btn = tk.Button(KO_frame, text = "Choose Model to Iterate", command = KO_iteration, font = ("times new roman",12))
...
def graph_design_point_losses(loss,option):
    ...
    color = ['blue','red']
    axs[0,0].pie(fracs, colors = color, startangle = 90, autopct = None, wedgeprops = dict(width = 0.4))
    axs[0,0].legend(["Nozzle = %.2f%%" % (fracs[0]*100), " Rotor  = %.2f%%" % (fracs[1]*100)], loc = 'center', fontsize = 12)
    axs[0,0].axis('equal')
    ...
    table_data = [[r"Y$_N$ =", "%.4f" % loss.Ytn],
                  ...
                  [r"$\eta_s$ =", "%.4f" % loss.eff]]
    table1 = axs[0,1].table(cellText = table_data, colLabels = None, edges = 'open', cellLoc = 'center', rowLoc = 'center', colLoc = 'center', loc = 'center')
    table1.auto_set_font_size(False)
    ...
    if (option == 3):
        ...
        color = ['gray', 'green']
    axs[1,0].pie(fracs, colors = color, startangle = 90, autopct = None, wedgeprops = dict(width = 0.4))
    axs[1,0].set_title("Nozzle", fontsize = 16)
    if (option == 3):
        axs[1,0].legend(['Y$_p$ = %.2f%%' % (fracs[0]*100), 'Y$_s$ = %.2f%%' % (fracs[1]*100), 'Y$_t$$_e$ = %.2f%%' % (fracs[2]*100)], loc = 'center', fontsize = 12)
    else:
        axs[1,0].legend(['Y$_p$ = %.2f%%' % (fracs[0]*100), 'Y$_s$ = %.2f%%' % (fracs[1]*100)], loc = 'center', fontsize = 12)
    axs[1,0].axis('equal')
    
    if (option == 3):
        total = loss.Ypr + loss.Ysr + loss.Ykr + loss.Yter
        fracs = [loss.Ypr/total, loss.Ysr/total, loss.Ykr/total, loss.Yter/total]
    ...
    axs[1,1].pie(fracs, colors = color, startangle = 90, autopct = None, wedgeprops = dict(width = 0.4))
    axs[1,1].set_title("Rotor", fontsize = 16)
    if (option == 3):
        axs[1,1].legend(['Y$_p$ = %.2f%%' % (fracs[0]*100), 'Y$_s$ = %.2f%%' % (fracs[1]*100), 'Y$_k$ = %.2f%%' % (fracs[2]*100), 'Y$_t$$_e$ = %.2f%%' % (fracs[3]*100)], loc = 'center', fontsize = 12)
    else:
        axs[1,1].legend(['Y$_p$ = %.2f%%' % (fracs[0]*100), 'Y$_s$ = %2.2f%%' % (fracs[1]*100), 'Y$_k$ = %.2f%%' % (fracs[2]*100)], loc = 'center', fontsize = 12)
    axs[1,1].axis('equal')
...
def tournier_genk(sigma_,beta_out):
    ...
    if (beta_out <= 63.2):
        ...
        B = 1.553*10**(-5.0)*beta_out**2.0 - 2.32*10**(-3.0)*beta_out + 8.02*10**(-2.0)
        C = -8.54*10**(-3.0)*beta_out + 0.238
        D = 4.83*10**(-5.0)*beta_out**2.0 + 2.83*10**(-4.0)*beta_out - 2.93*10**(-2.0)
    else:
    ...
    if (beta_out <= 60.0):
        sigma_min = -8.63*10**(-6.0)*beta_out**3.0 + 9.68*10**(-4.0)*beta_out**2.0 - 3.76*10**(-2.0)*beta_out + 1.272
        if (sigma_ >= sigma_min):
        ...
        sigma_min = -5.14*10**(-4.0)*beta_out**2.0 + 5.48*10**(-2.0)*beta_out - 0.798
        if (sigma_ >= sigma_min):
            n = 1.524*10**(-4.0)*beta_out**2.0 - 0.031*beta_out + 2.992
...
def graph_tournier_genk():
    ...
    for beta_out in ([40,50,60,65,70,75,80]):
        ...
        axs1.text(1.225, Yp_0_vector[len(Yp_0_vector)-1], "%d°" % beta_out, color = 'black')
        if (beta_out <= 70):
            axs2.plot(sc_vector, Yp_1_vector, 'k-')
            axs2.text(1.225, Yp_1_vector[len(Yp_1_vector)-1], "%d°" % beta_out, color = 'black')
    axs1.text(0.80, 0.09, "Relative outlet gas\n" + r"angle ($\alpha_2$ or $\beta_3$)", color = 'black')
    axs1.set(xlabel = "pitch/chord (s/c)", ylabel = r"Profile loss coefficient ($Y_p$)")
    axs1.set_xlim([0.3,1.3])
    axs1.set_ylim([0.0,0.10])
    axs1.spines['top'].set_visible(False)
    axs1.spines['right'].set_visible(False)
    axs2.text(0.80, 0.22, "Relative outlet gas\n" + r"angle ($\alpha_2$ or $\beta_3$)", color = 'black')
    axs2.set(xlabel = "pitch/chord (s/c)", ylabel = r"Profile loss coefficient ($Y_p$)")
...
def ainley_mathieson(blade,Gin,Gout,flow_in_angle,flow_out_angle):
    ...
    Yp0 = (Yp0_axial + ((blade.in_angle/flow_out_angle)**2.0)*(Yp0_impulse - Yp0_axial))*((blade.tc/0.2)**(blade.in_angle/flow_out_angle))
    ...
    data_lambd = pd.read_csv('lambd.csv',header=None, skiprows=[0,1])
    lambd_curve = interp1d(data_lambd[0],data_lambd[1],kind='linear')
    lambd_parameter = ((Gout.A*np.cos(np.radians(flow_out_angle)))/(Gin.A*np.cos(np.radians(flow_in_angle))))**2.0/(1.0 + 1/blade.rr)
    ...
    flow_mean_angle = np.arctan((np.tan(np.radians(flow_out_angle)) - np.tan(np.radians(flow_in_angle)))/2.0)
    Z = ((2.0*((np.tan(np.radians(flow_in_angle)) + np.tan(np.radians(flow_out_angle)))*np.cos(flow_mean_angle)))**2.0) * ((np.cos(np.radians(flow_out_angle)))**2.0/(np.cos(flow_mean_angle))**3.0)
...
def dunham_came(blade,Re,Ma,Yp_AM,Z,Xte,flow_out_angle):
    ...
    Ys0 = 0.0334*(1/blade.hc)*(np.cos(np.radians(flow_out_angle))/np.cos(np.radians(blade.in_angle)))*Z
...
def kacker_okapuu(blade,Re,Ma_in,Ma_out,Mah_in,p_in,p_out,Yp0,Ys0,Yk_DC,flow_out_angle):
    ...
    Yshock = 0.75*((Mah_in - 0.4)**(1.75))*(1/blade.rr)*(p_in/p_out)*(1 - (1 + ((gamma - 1)/2.0)*Ma_in**2.0)**(gamma/(gamma - 1)))/(1 - (1 + ((gamma - 1)/2.0)*Ma_out**2.0)**(gamma/(gamma - 1)))
    ...
    TE_coefficient = TEaxial + (TEimpulse - TEaxial)*(blade.in_angle/flow_out_angle)**2.0
    
    Yte = ((1 - ((gamma - 1)/2.0)*(Ma_out**2.0)*((1/(1 - TE_coefficient)) - 1))**((-gamma)/(gamma - 1)) - 1)/(1 - (1 + ((gamma - 1)/2.0)*(Ma_out**2.0))**((-gamma)/(gamma - 1)))
...
def loss_models(P1,P2,P3,Mach_1h,Machrel_2h):
    ...
    Yp0n, Zn, Xten, Ypn_AM, Ysn_AM, Ykn_AM, Ytn_AM = ainley_mathieson(nozzle,G1,G2,P1.alpha,P2.alpha)
    Yp0r, Zr, Xter, Ypr_AM, Ysr_AM, Ykr_AM, Ytr_AM = ainley_mathieson(rotor,G2,G3,P2.beta,P3.beta)

    lbdn_AM = Ytn_AM*P2.Ts/P2.T0
    lbdr_AM = Ytr_AM*P3.Ts/(P3.T + (P3.W**2.0)/(2.0*cp))
    
    eff_AM = 1/(1 + ((lbdr_AM*((P3.W**2.0)/(2.0*cp)) + (P3.T/P2.T)*lbdn_AM*((P2.V**2.0)/(2.0*cp)))/(P1.T0 - P3.T0)))
    ...
    Ys0n, Ypn_DC, Ysn_DC, Ykn_DC, Ytn_DC = dunham_came(nozzle,Ren,P2.Mach,Ypn_AM,Zn,Xten,P2.alpha)
    Ys0r, Ypr_DC, Ysr_DC, Ykr_DC, Ytr_DC = dunham_came(rotor,Rer,P3.Machrel,Ypr_AM,Zr,Xter,P3.beta)

    lbdn_DC = Ytn_DC*P2.Ts/P2.T0
    lbdr_DC = Ytr_DC*P3.Ts/(P3.T + (P3.W**2.0)/(2.0*cp))
    
    eff_DC = 1/(1 + ((lbdr_DC*((P3.W**2.0)/(2.0*cp)) + (P3.T/P2.T)*lbdn_DC*((P2.V**2.0)/(2.0*cp)))/(P1.T0 - P3.T0)))
    ...
    Ypn_KO, Ysn_KO, Ykn_KO, Yten_KO, Ytn_KO = kacker_okapuu(nozzle,Ren,P1.Mach,P2.Mach,Mach_1h,P1.p,P2.p,Yp0n,Ys0n,Ykn_DC,P2.alpha)
    
    #Mach_hub = P2.Machrel*(1.0 + 5.2*((1.0 - 1/rotor.rr)**2.2)) # If rotor, constante = 5.2
    Ypr_KO, Ysr_KO, Ykr_KO, Yter_KO, Ytr_KO = kacker_okapuu(rotor,Rer,P2.Machrel,P3.Machrel,Machrel_2h,P2.p,P3.p,Yp0r,Ys0r,Ykr_DC,P3.beta)
    
    lbdn_KO = Ytn_KO*P2.Ts/P2.T0
    lbdr_KO = Ytr_KO*P3.Ts/(P3.T + (P3.W**2.0)/(2.0*cp))
    
    eff_KO = 1/(1 + ((lbdr_KO*((P3.W**2.0)/(2.0*cp)) + (P3.T/P2.T)*lbdn_KO*((P2.V**2.0)/(2.0*cp)))/(P1.T0 - P3.T0)))
...
data_sc = pd.read_csv('optimum_sc.csv',header=None,skiprows=[0,1])
data_stress = pd.read_csv('permissible_stress.csv',header=None,skiprows=[0,1])
data_B = pd.read_csv('gas_bending_B_coefficient.csv',header=None,skiprows=[0,1])
data_n = pd.read_csv('gas_bending_n_coefficient.csv',header=None,skiprows=[0,1])
data_visc_air_temp = pd.read_csv('visc_air_temp.csv',header=None, skiprows=[0,1])
data_smith = pd.read_csv('smith_chart.csv',header=None,skiprows=[0,1])
...
myGui.geometry('{}x{}+{}+{}'.format(myGui_width, myGui_height, myGui_posx, myGui_posy))
